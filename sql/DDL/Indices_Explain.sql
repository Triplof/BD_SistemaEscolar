...
-- Alunos de uma turma com nome e CPF
EXPLAIN SELECT
    p.nome, p.sobrenome, p.cpf,
    m.situacao, t.nome_turma
FROM tb_matriculas m
JOIN tb_alunos     a ON a.fk_pessoa       = m.fk_pessoa
JOIN tb_pessoa     p ON p.pk_pessoa       = a.fk_pessoa
JOIN tb_turmas     t ON t.pk_turma        = m.fk_turma
WHERE m.fk_turma = 1;

-- Notas de um aluno em todas as disciplinas
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    av.descricao,
...
JOIN tb_disciplina     d  ON d.pk_disciplina  = av.fk_disciplina
JOIN tb_matricula_componentes mc ON mc.pk_vinculo = n.fk_vinculo
JOIN tb_matriculas     m  ON m.num_matricula  = mc.num_matricula
JOIN tb_pessoa         p  ON p.pk_pessoa      = m.fk_pessoa
WHERE m.fk_pessoa = 10;

-- Faltas por aluno e disciplina
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    SUM(f.quantidade) AS total_faltas
FROM tb_faltas        f
JOIN tb_matricula_componentes mc ON mc.pk_vinculo    = f.fk_vinculo
JOIN tb_matriculas    m  ON m.num_matricula  = mc.num_matricula
JOIN tb_pessoa        p  ON p.pk_pessoa      = m.fk_pessoa
JOIN tb_disciplina    d  ON d.pk_disciplina  = mc.fk_disciplina
GROUP BY p.pk_pessoa, d.pk_disciplina;

-- Grade horária de um professor
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    s.nome_sala,
    gh.dias_semana, gh.hora_inicio, gh.hora_fim
FROM tb_grade_horaria gh
JOIN tb_professor  pr ON pr.fk_pessoa      = gh.fk_professor
JOIN tb_pessoa     p  ON p.pk_pessoa       = pr.fk_pessoa
JOIN tb_disciplina d  ON d.pk_disciplina   = gh.fk_disciplina
JOIN tb_salas      s  ON s.pk_sala         = gh.fk_sala
WHERE gh.fk_professor = 5;
...
-- Folha de pagamento de um funcionário
EXPLAIN SELECT
    p.nome, p.sobrenome,
    fp.mes_referencia, fp.ano_referencia,
    fp.salario_base_atual, fp.total_proventos,
    fp.total_descontos, fp.valor_liquido
FROM tb_folha_pagamento fp
JOIN tb_funcionarios fu ON fu.fk_pessoa  = fp.fk_funcionario
JOIN tb_pessoa       p  ON p.pk_pessoa   = fu.fk_pessoa
WHERE fp.fk_unidade = 1
  AND fp.ano_referencia = 2024;

-- Fluxo de caixa por período e categoria
EXPLAIN SELECT
    fc.data_movimento,
    cf.nome_categoria,
    fc.tipo_movimento,
    fc.valor, fc.saldo_momento
FROM tb_fluxo_caixa           fc
JOIN tb_categorias_financeiras cf ON cf.pk_categoria = fc.fk_categoria
JOIN tb_unidades               u  ON u.pk_unidade    = fc.fk_unidade
WHERE fc.fk_unidade = 1
  AND fc.data_movimento BETWEEN '2024-01-01' AND '2024-12-31';
...
-- Alunos de uma turma (espera: type=ref, key=idx_matricula_turma)
EXPLAIN SELECT
    p.nome, p.sobrenome, p.cpf,
    m.situacao, t.nome_turma
FROM tb_matriculas m
...
-- Notas de um aluno (espera: key=idx_mc_matricula, idx_nota_vinculo)
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    av.descricao,
...
-- Faltas por aluno e disciplina (espera: key=idx_falta_vinculo)
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    SUM(f.quantidade) AS total_faltas
...
-- Grade horária de professor (espera: key=idx_grade_professor)
EXPLAIN SELECT
    p.nome, p.sobrenome,
    d.nome_disciplina,
    s.nome_sala,
...
-- Folha de pagamento (espera: key=idx_folha_unidade_periodo)
EXPLAIN SELECT
    p.nome, p.sobrenome,
    fp.mes_referencia, fp.ano_referencia,
    fp.salario_base_atual, fp.total_proventos,
...
-- Fluxo de caixa (espera: key=idx_fluxo_unidade_data)
EXPLAIN SELECT
    fc.data_movimento,
    cf.nome_categoria,
    fc.tipo_movimento,
